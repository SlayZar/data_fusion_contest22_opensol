...
Суть эмбеддингов - доля кликов и транзакций пользователя в каждый час (0-23).

После этого обучается простая модель катбуст на всех данных на дефолтных параметрах.

Данный простой подход позволяет за 6 минут считать данные и получить обученную модель, которая на текущий момент позволяет попасть в топ-10 на публичном лидерборде по задачам Matching и Puzzle.
